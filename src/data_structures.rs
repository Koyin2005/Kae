...
#[derive(Debug)]
pub struct IndexVec<Index, Value>
where
    Index: IntoIndex,
{
    data: Vec<Value>,
...
impl<Index: IntoIndex, Value> IndexVec<Index, Value> {
    ...
    pub fn from_fn(mut f: impl FnMut(Index) -> Value, len: usize) -> Self
    where
        Value: Clone,
    {
        Self {
            data: Vec::from_iter((0..len).map(|i| f(Index::new(i)))),
            _phantom: PhantomData,
        }
    }
    pub fn from(value: Value, len: usize) -> Self
    where
        Value: Clone,
    {
        Self {
    ...
    pub fn index_value_iter(&self) -> impl '_ + std::iter::Iterator<Item = (Index, &'_ Value)> {
        self.data
            .iter()
            .enumerate()
            .map(|(i, value)| (Index::new(i), value))
    }
    pub fn index_value_iter_mut(
        &mut self,
    ) -> impl '_ + std::iter::Iterator<Item = (Index, &'_ mut Value)> {
        self.data
            .iter_mut()
            .enumerate()
            .map(|(i, value)| (Index::new(i), value))
    ...
    pub fn indices(&self) -> impl Iterator<Item = Index> + use<Index, Value> {
