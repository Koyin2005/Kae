...
use crate::{
    ...
    frontend::{
        ast_lowering::hir::{self, DefId, LiteralKind},
        thir,
...
fn test_trees_from(
    ...
    let base_test = match &pattern.kind {
        ...
        thir::PatternKind::Tuple(fields) => {
            for (i, field) in fields.iter().enumerate() {
                test_trees_from(
                    ...
                    place
                        .clone()
                        .project(mir::PlaceProjection::Field(FieldIndex::new(i))),
                    &mut subtests,
                );
            }
            None
        ...
        thir::PatternKind::Struct(_, _, fields) => {
            for field_pattern in fields {
                test_trees_from(
                    ...
                    place
                        .clone()
                        .project(mir::PlaceProjection::Field(field_pattern.field)),
                    &mut subtests,
                );
            }
            None
        ...
        &thir::PatternKind::Variant(id, _, variant, ref fields) => {
            let projected_place = place.clone().project(mir::PlaceProjection::Variant(
                context.get_variant_by_index(id, variant).name.index,
                variant,
            ));
            for (i, field) in fields.iter().enumerate() {
                test_trees_from(
...
impl<'a> BodyBuild<'a> {
    fn test_result(place: &Place, test: &Test, branch: &mut MatchBranch) -> Option<TestResult> {
        let (index, match_test) = branch
            .tests
            .iter()
            .enumerate()
            .find(|(_, branch_test)| &branch_test.base_test.place == place)?;
    ...
    fn group_branches<'b, 'c>(
        ...
        place: &Place,
        test: &Test,
        mut branches: &'b mut [&'c mut MatchBranch],
    ) -> (
        IndexMap<TestResult, Vec<&'b mut MatchBranch>>,
        &'b mut [&'c mut MatchBranch],
    ) {
    ...
    fn perform_test(
        ...
        place: Place,
        test: Test,
        targets: IndexMap<TestResult, BlockId>,
        otherwise_block: BlockId,
    ) {
        let get_target =
            |result: TestResult| targets.get(&result).copied().unwrap_or(otherwise_block);
        match test {
            Test::If => {
                self.terminate(mir::Terminator::Switch(
                    mir::Operand::Load(place),
                    Box::new([(0, get_target(TestResult::Failure))]),
                    get_target(TestResult::Success),
                ));
            }
            Test::SwitchInt => {
                let targets = targets
            ...
            Test::Eq(constant) => {
                ...
                self.assign_stmt(
                    ...
                    mir::RValue::Binary(
                        hir::BinaryOp::Equals,
                        Box::new((Operand::Load(place), Operand::Constant(constant))),
                    ),
                );
                self.terminate(mir::Terminator::Switch(
                    mir::Operand::Load(is_equal.into()),
                    Box::new([(0, fail_block)]),
                    success_block,
                ));
            ...
            Test::SwitchVariant(id) => {
                ...
                let targets = (0..self.context.expect_variants(id).len())
                    ...
                    .collect();
                self.terminate(mir::Terminator::Switch(
                    mir::Operand::Load(discriminant.into()),
                    targets,
                    get_target(TestResult::Failure),
                ));
            }
        }
    }
    fn build_match(
        ...
        let (remaining, otherwise_block) = match branches {
            ...
            branches => {
                ...
                let otherwise_block = self.new_block();
                let targets: IndexMap<_, _> = branches
                    .into_iter()
                    .map(|(test, mut branch)| {
    ...
    pub fn lower_match<'b>(
        ...
        scrutinee: Place,
        _: &Type,
        arms: impl Iterator<Item = &'b thir::Arm>,
    ) {
        let (tests, bodies): (Vec<_>, Vec<_>) = arms
            .into_iter()
            .map(|arm| {
        ...
        let mut branches = tests
            .into_iter()
            .map(|tests| MatchBranch {
                tests,
                success: None,
            })
            .collect::<Vec<_>>();
        let otherwise_block = self.build_match(
            0,
            &mut branches.iter_mut().collect::<Box<[_]>>(),
            self.current_block,
        );
        self.current_block = otherwise_block;
