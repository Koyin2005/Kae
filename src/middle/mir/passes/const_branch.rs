use crate::{
    ...
    middle::mir::{
        Body, Operand, Terminator,
        passes::{MirPass, Patch},
    },
};
...
impl MirPass for ConstBranch {
    ...
    fn run_pass(&self, _: &TypeContext, body: &mut Body) {
        ...
        'a: for (block_id, block) in body.blocks.index_value_iter() {
            if let Terminator::Switch(Operand::Constant(constant), targets, else_branch) =
                block.expect_terminator()
            {
            ...
            if let Terminator::Assert(Operand::Constant(constant), _, target) =
                block.expect_terminator()
            {
                if let Some(const_value) = constant.eval_to_scalar() {
                    if const_value == 1 {
