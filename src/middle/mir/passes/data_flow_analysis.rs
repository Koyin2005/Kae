...
use crate::{data_structures::IndexVec, define_id, frontend::typechecking::context::TypeContext, identifiers::{FieldIndex, VariantIndex}, middle::mir::{basic_blocks::BasicBlockInfo, traversal, visitor::{PlaceContext, Visitor}, BlockId, Body, Local, Location, Place, PlaceProjection, Stmt, Terminator}};
...
pub trait Analysis : Sized{
    ...
    fn perform_terminator_effect(&mut self, terminator: &Terminator, _state: &mut Self::Domain) -> Box<[BlockId]>{
    ...
    fn iterate_to_fixed_point(&mut self, body: &Body) -> Results<Self::Domain>{
        ...
        let mut results: IndexVec<BlockId, _> = IndexVec::from(self.bottom_value(), body.blocks.len());
        ...
        while let Some(block) = queue.pop_front() {
            ...
            for succ in self.perform_terminator_effect(body.blocks[block].expect_terminator(),&mut state) {
                if results[succ].join(&state){
...
pub trait ResultVisitor<A:Analysis> {
    ...
    fn before_terminator_effect(&mut self,_location:Location,_terminator:&Terminator,_state:&A::Domain){}
    fn after_terminator_effect(&mut self,_location:Location,_terminator:&Terminator,_state:&A::Domain){}
}

pub fn visit_reachable_results<A:Analysis>(visitor:&mut impl ResultVisitor<A>,body:&Body, blocks: impl Iterator<Item = BlockId>,analysis:&mut A,mut results: Results<A::Domain>){
    for block in blocks{
        for (i, stmt) in body.blocks[block].stmts.iter().enumerate() {
            let location = Location{statement_index:i,basic_block:block};
            visitor.before_statement_effect(location, stmt, &mut results[block]);
            analysis.perform_statement_effect(stmt, &mut results[block]);
            visitor.after_statement_effect(location, stmt, &mut results[block]);
        }
        let location = Location { basic_block: block, statement_index: body.blocks[block].stmts.len() };
...
pub fn visit_results<A:Analysis>(visitor:&mut impl ResultVisitor<A>,body:&Body, blocks: impl Iterator<Item = BlockId>,analysis:&mut A,mut results: Results<A::Domain>){
    for block in blocks{
        for (i, stmt) in body.blocks[block].stmts.iter().enumerate() {
            let location = Location{statement_index:i,basic_block:block};
            visitor.before_statement_effect(location, stmt, &mut results[block]);
...
impl<V:HasBottom> StateInfo<V>{
    pub fn new() -> Self{
        Self { map: FxHashMap::default() , bottom : V::BOTTOM}
    }
...
impl Map {
    pub fn for_each_place_in<T:Clone>(&self, place: PlaceIndex, value: T, map: &mut impl FnMut(PlaceIndex,Projection,&T) -> T, f: &mut impl FnMut(PlaceIndex,T)){
    ...
    pub fn get_projection_for(&self, place: PlaceIndex, projection: Projection) -> Option<PlaceIndex>{
    ...
    pub fn new(body: &Body, context: &TypeContext) -> Self {
        ...
        for local in body.args(){
            map.locals[local] = Some(map.next_place(PlaceInfo { projection: None, children: vec![] }));
        }
        PlaceCollector { map: &mut map ,body, context}.visit_body(body);
        map
    }
...
impl Visitor for PlaceCollector<'_> {
    fn visit_place(&mut self, place: &Place, _: PlaceContext, _: Location) {
        ...
        if place.type_of(self.context, self.body).is_enum(){
            let tag_place = self.map.next_place(PlaceInfo { projection: Some(Projection::Tag), children: vec![] });
            self.map.projections.insert((current_place,Projection::Tag), tag_place);
            self.map.tracked_places[current_place].children.push(tag_place);
        }
    }
}