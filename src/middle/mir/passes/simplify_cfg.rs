...
impl<'a> CfgSimplifier<'a>{
    pub fn new(body:&'a mut Body) -> Self{
        ...
        Self {basic_blocks:&mut body.blocks, predecessors }
    }
    ...
    fn merge_successor(&mut self,merged_blocks: &mut Vec<BlockId>, terminator: &mut Terminator) -> bool{
    ...
    fn simplify(&mut self){
        loop{
            ...
            for block_id in self.basic_blocks.indices(){
                ...
                let mut terminator = self.basic_blocks[block_id].terminator.take().expect("There should be a terminator");
                ...
                let statements_to_merge : usize = merged_blocks.iter().map(|&block| self.basic_blocks[block].stmts.len()).sum();
