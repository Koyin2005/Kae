...
use crate::{
    ...
    frontend::{
        ...
        typechecking::types::{AdtKind, ArraySize, ConstantSize, Type},
    },
    ...
    middle::mir::{
        ...
        }, visitor::{MutVisitor, Visitor}, AggregrateConstant, BlockId, Constant, ConstantKind, ConstantNumber, ConstantValue, Location, Operand, Place, PlaceProjection, RValue, Stmt, Terminator
    },
};
...
impl ConstAnalysis<'_> {
                        ...
                        for &projection in place.projections.iter() {
                            ...
                            place_index = match projection {
                                ...
                                PlaceProjection::Variant(_, variant_index) => {
                                    self.map.get_projection_for(
                                        place_index,
                                        Projection::Variant(variant_index),
                                    )
                                }
                                ...
                                PlaceProjection::Index(index_local) => {
                                    ...
                                    let ConstantNumber::Int(index_value) =
                                        index_value.as_number()?
                                    else {
                                        return None;
                                    };
                                    println!("RAHH !");
                                    value =
                                        Some(eval_index(&current_value, index_value as u64).ok()?);
                                    continue;
                                }
                            }?;
                        }
    ...
    fn handle_rvalue(
        ...
        match rvalue {
            ...
            RValue::Array(ty, elements) => {
                ...
                CompleteSet::Elem(ConstantValue {
                    ty: Type::new_array(ty.clone(), ConstantSize::Value(ArraySize::new(const_elements.len()))),
            ...
            RValue::Len(place) => {
                ...
                match self.try_constant(place, state) {
                    CompleteSet::Elem(val) => {
                        ...
                        CompleteSet::Elem(Constant::int(elements.len() as i64))
                    }
                    val => val,
                }
            }
            RValue::Unary(op, operand) => {
                ...
                eval_unary_op(*op, constant).map(|val| val.into()).map_or(CompleteSet::Top, CompleteSet::Elem)
            }
                ...
                match (left, right) {
                    (CompleteSet::Elem(left), CompleteSet::Elem(right)) => {
                        eval_binary_op(*op, left, right)
                            .ok().map(|val| val.into())
                            .map_or(CompleteSet::Top, CompleteSet::Elem)
                    }
                    (CompleteSet::Elem(const_val), other)
                    | (other, CompleteSet::Elem(const_val)) => {
                        let Some(ConstantNumber::Int(val)) = const_val.as_number() else {
                            return CompleteSet::Top;
                        };
                        match (op, val) {
                            (BinaryOp::Multiply, 0) => {
                                CompleteSet::Elem(Constant::int(0))
                            }
            ...
            RValue::Tuple(element_types, elements) => {
                ...
                for operand in elements
                    .iter()
                    .map(|operand| self.operand_as_constant(operand, state))
                {
                    let CompleteSet::Elem(constant) = operand else {
                        return operand;
                    };
                ...
                CompleteSet::Elem(ConstantValue {
                    ...
                    kind: ConstantKind::Aggregrate(Box::new(AggregrateConstant::Tuple(
                        const_fields.into_boxed_slice(),
                    ))),
                }.into())
            }
            ...
            RValue::Adt(adt_info, fields) => {
                ...
                CompleteSet::Elem(ConstantValue {
                    ...
                    kind: ConstantKind::Aggregrate(Box::new(AggregrateConstant::Adt(
                        *id,
                        generic_args.clone(),
                        *variant,
                        const_fields.into_boxed_slice(),
    ...
    fn handle_assign(
        ...
        place: &Place,
        rvalue: &RValue,
        state: &mut State<CompleteSet<Constant>>,
    ) {
        ...
        let flood = match rvalue {
            RValue::Adt(adt_info, fields) => {
                ...
                let base_place = variant
                    .and_then(|variant| {
                        self.map
                            .get_projection_for(place_index, Projection::Variant(variant))
                    })
                    .unwrap_or(place_index);
                for (field, operand) in fields.index_value_iter() {
                    let Some(projection_place) = self
                        .map
                        .get_projection_for(base_place, Projection::Field(field))
                    else {
                        continue;
                    };
                    self.assign_val(
                        state,
                        projection_place,
                        self.operand_as_constant(operand, state),
                    );
                }
                    ...
                    && let Some(tag) = self.map.get_projection_for(place_index, Projection::Tag)
                {
                    self.assign_to_place(
                        state,
                        tag,
                        CompleteSet::Elem(Constant::int(variant.as_index() as i64)),
                    );
                }
                false
            }
            RValue::Tuple(_, fields) => {
                for (field, operand) in
                    fields
                        .iter()
                        .enumerate()
                        .map(|(field_index, field_operand)| {
                            (FieldIndex::new(field_index), field_operand)
                        })
                {
                    let Some(projection_place) = self
                        .map
                        .get_projection_for(place_index, Projection::Field(field))
                    else {
                        continue;
                    };
                    self.assign_val(
    ...
    fn assign_val(
        ...
        self.map.for_each_place_in(
            place,
            value,
            &mut |_, projection, value| {
                let value = match value {
                    CompleteSet::Elem(value) => value,
                ...
                CompleteSet::Elem(match projection {
                    Projection::Field(field) => {
                        let Some(
                            AggregrateConstant::Adt(_, _, _, fields)
                            | AggregrateConstant::Tuple(fields),
                        ) = value.as_aggregrate()
                        else {
                            return CompleteSet::Top;
                        };
                    ...
                    Projection::Tag => {
                        let Some(AggregrateConstant::Adt(_, _, Some(variant), _)) =
                            value.as_aggregrate()
                        else {
                            return CompleteSet::Top;
                        };
                        Constant::int(variant.as_index() as i64)
                    }
                    Projection::Variant(_) => value.clone(),
                })
            },
            &mut |place, val| {
        ...
        *state = State::new();
        for arg in body.args() {
            self.assign_val(
                ...
                self.map
                    .get(&arg.into())
                    .expect("Every arg should have a place"),
                CompleteSet::Top,
            );
        }
    }
...
impl<'a, 'b, 'c> ResultVisitor<ConstAnalysis<'a>> for ConstVisitor<'b, 'c> {
    fn before_statement_effect(
        ...
        location: Location,
        stmt: &Stmt,
        state: &<ConstAnalysis<'a> as Analysis>::Domain,
    ) {
        Collector {
            patch: self.patch,
            before_stmt: true,
        ...
        .visit_stmt(stmt, location);
    }
    fn after_statement_effect(
        &mut self,
        location: Location,
        stmt: &Stmt,
        state: &<ConstAnalysis<'a> as Analysis>::Domain,
...
impl Visitor for Collector<'_, '_> {
    ...
    fn visit_assign(&mut self, lvalue: &Place, rvalue: &RValue, location: Location) {
        if !self.before_stmt
            && let Some(place) = self.map.get(lvalue)
...
impl MutVisitor for ConstPatch<'_> {
    ...
    fn visit_stmt(&mut self, stmt: &mut Stmt, location: Location) {
        if let Stmt::Assign(place, rvalue) = stmt
            && let Some(place) = self.map.get(place)
