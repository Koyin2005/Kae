...
use crate::{
    ...
    middle::mir::{self, Block, BlockId, Body, Terminator},
};
...
impl Patch {
    pub fn new() -> Self {
        Patch {
            extra_block_map: Default::default(),
    ...
    pub fn apply(mut self, body: &mut Body) {
        ...
        for (block_id, block) in self
            .new_blocks
            .into_iter()
            .enumerate()
            .map(|(block_index, block)| (BlockId(block_index as u32), block))
        {
            self.extra_block_map
                .insert(block_id, BlockId(next_block_id));
