use crate::{
    ...
    define_id,
    middle::mir::{BlockId, basic_blocks::BasicBlockInfo},
};
...
pub fn dominators(bb_count: usize, blocks: &BasicBlockInfo) -> Dominators {
    ...
    let mut real_to_preorder: IndexVec<BlockId, Option<PreorderIndex>> =
        IndexVec::from(None, bb_count);
    ...
    let mut ancestor: IndexVec<PreorderIndex, Option<PreorderIndex>> =
    ...
    Dominators {
        immediate_dominators: block_idoms,
    }
}
...
fn eval(
    ...
    sdom: &IndexVec<PreorderIndex, PreorderIndex>,
    mut v: PreorderIndex,
) -> PreorderIndex {
    let mut u = v;
