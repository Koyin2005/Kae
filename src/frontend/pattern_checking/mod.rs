...
impl PatternRow{
    ...
    pub fn specialize(&self, constructor: Constructor, fields:&[Type]) -> Option<Self>{
        ...
        let new_row = Self { patterns: new_fields.into_iter().chain(self.patterns[1..].iter().cloned()).collect() };
        Some(new_row)
    }
    pub fn unspecialize(&self, ty: Type, constructor: Constructor, arity: usize) -> Self{
        let fields = &self.patterns[0..arity];
        Self { patterns: std::iter::once(Pattern{ty,constructor,fields:fields.to_vec()}).chain(self.patterns[arity..].iter().cloned()).collect() }
    }
}
#[derive(Clone,Debug)]
...
impl PatternMatrix{
    fn new(rows : impl Iterator<Item = PatternRow>,column_types:Vec<Type>) -> Self{
        Self { rows: rows.collect(),column_types}
    }
    fn new_empty(column_types:Vec<Type>) -> Self{
        Self { rows: Vec::new(),column_types}
    }

    fn specialize(&self,constructor : Constructor,fields:&[Type]) -> Self{
        let new_rows = self.rows.iter().filter_map(|row|{
            row.specialize(constructor,fields)
        }).collect();
        ...
        Self { rows: new_rows , column_types : columns }
    }
}
...
impl<'a> PatternChecker<'a>{
    ...
    fn constructors(&self, ty: &Type) -> ConstructorSet{
        match ty{
            ...
            Type::Param(_,_) | Type::Error | Type::Float | Type::String | Type::Array(_) | Type::Function(_,_)  => ConstructorSet::Infinite,
        ...
        for constructor in constructors{
            ...
            let new_witnesses = if matches!(constructor,Constructor::Missing){
                ...
                for &missing_constructor in &missing{
                    ...
                    let wild_card_filled_pattern = Pattern { ty: ty.clone(), constructor: missing_constructor, fields:  fields.into_iter().map(|field|{
                            Pattern{
                                ty:field,
                                constructor:Constructor::Wildcard,
                                fields:vec![]
    ...
    pub fn check_exhaustive(self,patterns:Vec<Pattern>,ty:&Type) -> ExhaustiveResults{
        let matrix = self.usefulness(PatternMatrix::new(patterns.into_iter().map(|pattern| PatternRow::new(vec![pattern])),vec![ty.clone()]),0);
        ExhaustiveResults { missing_patterns: matrix.rows.into_iter().map(|mut row| row.take_first()).collect() }
    }
}
