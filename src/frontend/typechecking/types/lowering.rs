...
impl<'a> TypeLower<'a>{
    ...
    pub fn lower_sig<'b>(&self, params: impl Iterator<Item = &'b hir::Type>, return_type : Option<&'b hir::Type>) -> FuncSig{
        FuncSig { params: params.map(|param| self.lower_type(param)).collect(), return_type: return_type.map(|return_type| self.lower_type(return_type)).unwrap_or_else(Type::new_unit) }
    }
    ...
    pub fn lower_generic_args_of_path(&self,path:&hir::Path) -> GenericArgs{
        let generic_args = (|| Some(
                match path.final_res{
                    ...
                    res @ Resolution::Definition(DefKind::Function|DefKind::Struct|DefKind::Enum, _) => {
                        let segment = path.segments.iter().rev().find(|segment|{
                            segment.res == res
                        })?;
                        &segment.args
    ...
    fn lower_path(&self,path:&hir::Path) -> Type{
        match path.final_res{
            ...
            hir::Resolution::Definition(hir::DefKind::Enum,id) => Type::Adt(self.lower_generic_args_of_path(path), id,AdtKind::Enum),
            hir::Resolution::Definition(hir::DefKind::Struct,id) => Type::Adt(self.lower_generic_args_of_path(path), id,AdtKind::Struct),
        ...
        match path.final_res{
            hir::Resolution::Definition(hir::DefKind::Variant,id) => {
                let segment = path.segments.iter().position(|segment|{
                    segment.res == Resolution::Definition(hir::DefKind::Enum, self.context.expect_owner_of(id))
                });
