...
use crate::{
    ...
    frontend::{
        ast_lowering::{SymbolIndex, SymbolInterner, hir::Ident},
        parsing::ast::{self, ExprNode, FunctionSig, Impl, NodeId},
...
use super::{
    hir::{BuiltinKind, DefId, DefIdMap, DefIdProvider, DefKind, PrimitiveType, Resolution},
...
impl NamesFound {
    pub fn expect_base_scope_for(&self, node: NodeId) -> ScopeId {
        self.scope_map
            .get(&node)
            .expect("Expected scopes for node id ")
            .last()
            .copied()
            .expect("Expect base scope")
    }
...
impl<'b, 'a: 'b> NameFinder<'b> {
    pub fn new(
        interner: &'b mut SymbolInterner,
        def_id_provider: &'b mut DefIdProvider,
    ...
    fn define_bindings(
        &mut self,
        base_pattern_id: NodeId,
        bindings: &BTreeMap<NodeId, Ident>,
        seen_symbols: &BTreeMap<SymbolIndex, NodeId>,
    ) {
        let mut all_variables = Vec::new();
        for (node, &ident) in bindings {
            if seen_symbols
                .get(&ident.index)
                .is_some_and(|node_id| node_id != node)
            {
                self.error(
                    format!("Repeated binding '{}'.", self.interner.get(ident.index)),
                    ident.span,
                );
                continue;
            }
        ...
        self.info
            .variable_def_map
            .insert(base_pattern_id, all_variables);
    }
    ...
    fn begin_scope(&mut self, kind: ScopeKind) {
        let (_, new_scope) = self
            .scope_tree
            .add_scope(Scope::new(kind), self.current_scope);
    ...
    fn add_node_to_def(&mut self, id: NodeId, def_id: DefId) {
        assert!(
            self.info.node_to_def_map.insert(id, def_id).is_none(),
            "There should be 1 def id for every node id"
        );
    }
    ...
    fn find_names_in_expr(&mut self, expr: &'b ast::ExprNode) {
        let id = expr.id;
        match &expr.kind {
            ast::ExprNodeKind::Array(elements)
            | ast::ExprNodeKind::Print(elements)
            | ast::ExprNodeKind::Tuple(elements) => elements.iter().for_each(|element| {
                self.find_names_in_expr(element);
            }),
            ast::ExprNodeKind::BinaryOp { op: _, left, right }
            | ast::ExprNodeKind::Logical { op: _, left, right }
            | ast::ExprNodeKind::While {
                condition: left,
                body: right,
            }
            | ast::ExprNodeKind::Index {
                lhs: left,
                rhs: right,
            } => {
            ...
            ast::ExprNodeKind::Unary {
                op: _,
                operand: expr,
            }
            | ast::ExprNodeKind::Grouping(expr)
            | ast::ExprNodeKind::Property(expr, _)
            | ast::ExprNodeKind::Instantiate { lhs: expr, args: _ } => {
                self.find_names_in_expr(expr)
            }
            ast::ExprNodeKind::Call {
                callee: first,
                args,
            }
            ...
            ast::ExprNodeKind::StructInit { path: _, fields } => fields
                .iter()
                .for_each(|(_, expr)| self.find_names_in_expr(expr)),
    ...
    fn find_names_in_pattern(
        ...
        match &pattern.kind {
            ...
            | ast::ParsedPatternNodeKind::Path(_)
            | ast::ParsedPatternNodeKind::Infer(_) => (),
        }
    }
    fn find_generic_params(
        &mut self,
        owner: DefId,
        generic_params: Option<&'b ast::ParsedGenericParams>,
    ) {
            ...
            generic_params
                ...
                .map(|param| {
                    ...
                    if !seen_generic_params.insert(index) {
                        self.error(
                            format!("Repeated generic param '{}'.", self.interner.get(index)),
                            param.0.location,
                        );
                    }
    ...
    fn find_names_in_function(
        &mut self,
        id: NodeId,
        sig: &'b FunctionSig,
        body: Option<&'b ExprNode>,
    ) {
        self.begin_scope(ScopeKind::Function(id));
        self.prev_last_local_variables
            .push(std::mem::replace(&mut self.next_local_variable, 0));
        let mut bindings = BTreeMap::new();
        let mut seen_symbols = BTreeMap::new();
    ...
    fn find_names_in_item(&mut self, item: &'a ast::Item) {
        match item {
            ast::Item::Enum(enum_def) => {
                ...
                let variants = enum_def
                    ...
                    .map(|variant| {
                        ...
                        if self
                            ...
                            .add_binding(name.index, Resolution::Definition(DefKind::Variant, id))
                            .is_some()
                        {
                            self.error(
                                format!(
                                    "Repeated variant '{}'.",
                                    self.interner.get(variant.name.content)
                                ),
                                name.span,
                            );
                        }
                ...
                self.namespaces.define_namespace(
                    Resolution::Definition(DefKind::Enum, enum_def_id),
                    name_scope,
                );
                let index = enum_def.name.content;
                if self
                    .get_current_scope_mut()
                    .add_binding(index, Resolution::Definition(DefKind::Enum, enum_def_id))
                    .is_some()
                {
                    self.error(
                        format!(
                            "Repeated item '{}'.",
                            self.interner.get(enum_def.name.content)
                        ),
                        enum_def.name.location,
                    );
                }
            }
            ast::Item::Struct(struct_def) => {
                ...
                self.info
                    .structs
                    .insert(struct_def_id, Record { name, fields });
                    ...
                    .is_some()
                {
                    self.error(
                        format!(
                            "Repeated item '{}'.",
                            self.interner.get(struct_def.name.content)
                        ),
            ...
            ast::Item::Fun(function_def) => {
                ...
                self.find_names_in_function(
                    function_def.id,
                    &proto.sig,
                    Some(&function_def.function.body),
                );
            ...
            ast::Item::Impl(impl_) => {
                ...
                self.get_current_scope_mut()
                    .add_binding(self_symbol, Resolution::SelfType(impl_def_id));
                let mut methods = Vec::new();
                for method in &impl_.methods {
                    let method_def_id = self.find_names_in_method(
                        method.id,
                        method.function.proto.name,
                        method.function.proto.generic_params.as_ref(),
                        &method.function.proto.sig,
                        Some(&method.function.body),
                    );
