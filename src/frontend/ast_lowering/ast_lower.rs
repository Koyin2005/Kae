...
use super::{
    SymbolInterner,
    hir::{
        self, Body, BodyOwner, DefId, DefIdMap, DefKind, FunctionDef, GenericArg, Hir, HirId,
        Ident, Item, LiteralKind, PatternKind, Resolution, Type,
    },
    name_finding::{self, NameScopes},
    resolve::Resolver,
...
impl<'a> AstLowerer<'a> {
    pub fn new(
        ...
        Self {
            ...
            resolver: RefCell::new(Resolver::new(
                name_scopes.namespaces,
                name_scopes.scope_tree,
            )),
    ...
    fn begin_scope(&mut self, id: NodeId) {
        self.prev_scopes
            .borrow_mut()
            .push(self.resolver.borrow().current_scope_id());
        let scopes = self
            .name_info
            .scope_map
            .get_mut(&id)
            .expect("There should be a scope for this id");
        self.resolver
            .borrow_mut()
            .set_current_scope(scopes.pop().expect("There should be a scope here"));
    }
    fn end_scope(&self) {
        let prev_scope = self
            .prev_scopes
            .borrow_mut()
            .pop()
            .expect("There should be a scope here.");
    ...
    fn lower_generic_args(
        &self,
        args: Option<&ast::ParsedGenericArgs>,
    ) -> Result<Vec<GenericArg>, LoweringErr> {
    ...
    fn lower_generic_params(
        &mut self,
        owner: DefId,
        generics: Option<ast::ParsedGenericParams>,
    ) -> Result<Generics, LoweringErr> {
        ...
        let generics = if let Some(ast::ParsedGenericParams(_, params)) = generics {
                ...
                params.into_iter().zip(&self.name_info.generics[owner])
            {
                if !seen_names.insert(name.index) {
                    self.error(
                        format!(
                            "Repeated generic param '{}'.",
                            self.symbol_interner.get(name.index)
                        ),
                        name.span,
                    );
                    had_error = true;
    ...
    fn lower_path(&self, path: &ast::Path) -> Result<hir::QualifiedPath, LoweringErr> {
        let resolutions = self
            .resolver
            .borrow()
            .resolve_path(path.segments.iter().map(|segment| segment.name.content));
        if resolutions.is_empty() {
            let name = path
                .segments
                .first()
                .expect("Should be at least 1 segment")
                .name;
            self.error(
                format!(
                    "Cannot find '{}' in scope.",
                    self.symbol_interner.get(name.content)
                ),
                name.location,
            );
        ...
        let segments = resolutions
            .into_iter()
            .zip(&path.segments)
            .map(|(resolution, segment)| {
                Ok(hir::PathSegment {
                    res: resolution,
            ...
            .into_iter()
            .collect::<Result<Vec<_>, _>>()?;
        let full_path = hir::QualifiedPath::FullyResolved(hir::Path {
            span: path.location,
            final_res,
            segments,
        });
        ...
        let (Resolution::Primitive(_)
        | Resolution::SelfType(_)
        | Resolution::Definition(hir::DefKind::Enum | hir::DefKind::Struct, _)) = final_res
        else {
            ...
            self.error(
                format!(
                    "Cannot find '{}' in '{}'.",
                    self.symbol_interner.get(last.content),
                    base_path
                ),
                span,
            );
        ...
        for offset in 0..unresolved_segments {
            ...
            full_path = hir::QualifiedPath::TypeRelative(
                Box::new(Type {
                    span: full_path.span(),
                    kind: hir::TypeKind::Path(full_path),
                }),
                hir::PathSegment {
                    res: Resolution::None,
                    ident: segment.name.into(),
        ...
        let (kind, span) = match ty {
            ...
            &ast::Type::Tuple(span, ref elements) => {
                let element_types = elements
                    ...
                    .collect::<Vec<_>>();
                (
                    hir::TypeKind::Tuple(element_types.into_iter().collect::<Result<_, _>>()?),
                    span,
                )
            }
            &ast::Type::Fun(span, ref params, ref return_type) => {
                ...
                let return_type = return_type
                    .as_ref()
                    .map(|return_type| self.lower_type(return_type).map(Box::new))
                    .map_or(Ok(None), |result| result.map(Some))?;
                (
                    hir::TypeKind::Function(
                        param_types.into_iter().collect::<Result<_, _>>()?,
                        return_type,
                    ),
                    span,
                )
    ...
    fn lower_function(
        ...
        body: ast::ExprNode,
        is_anon: bool,
    ) -> Result<hir::Function, LoweringErr> {
        ...
        Ok(hir::Function {
            ...
            body: {
                ...
                self.body_owners.push(if is_anon {
                    BodyOwner::AnonFunction(def_id)
                } else {
                    BodyOwner::Function(def_id)
                });
                body
            },
        })
    }
    fn define_bindings_and_lower_pattern(
        &mut self,
        pattern: ast::ParsedPatternNode,
    ) -> Result<hir::Pattern, LoweringErr> {
        ...
        for variable in variables {
            ...
            ) = self.name_info.variables[variable];
            self.resolver
                .borrow_mut()
                .current_scope_mut()
                .add_binding(name, hir::Resolution::Variable(variable));
        }
        ...
        let (kind, span) = match pattern.kind {
            ast::ParsedPatternNodeKind::Is(name, matched_pattern) => {
                ...
                let id = *self
                    ...
                    .variable_defs
                    .get(&pattern.id)
                    .expect("All bindings should be defined before lowered");
                (
                    PatternKind::Binding(id, name, Some(Box::new(matched_pattern?))),
                    span,
                )
            }
            ast::ParsedPatternNodeKind::Literal(literal) => {
                let literal = match literal {
                    ...
                    ast::LiteralKind::String(string) => {
                        LiteralKind::String(self.symbol_interner.intern(string))
                    }
                };
            ...
            ast::ParsedPatternNodeKind::Name(name) => {
                let name = self.intern_symbol(Symbol {
                    content: name,
                    location: span,
                });
                let id = *self
            ...
            ast::ParsedPatternNodeKind::Tuple(elements) => {
                let elements: Vec<_> = elements
                    ...
                    .collect();
                (
                    PatternKind::Tuple(elements.into_iter().collect::<Result<Vec<_>, _>>()?),
                    span,
                )
            }
            ast::ParsedPatternNodeKind::TupleStruct(path, fields) => {
                ...
                let fields = fields
                    ...
                    .collect::<Vec<_>>();
                (
                    PatternKind::Variant(path?, fields.into_iter().collect::<Result<Vec<_>, _>>()?),
                    span,
                )
            }
            ast::ParsedPatternNodeKind::Struct { path, fields } => {
                let path: Result<hir::InferOrPath, LoweringErr> = match path.infer_path {
                    ast::InferPathKind::Infer(symbol) => Ok(hir::InferOrPath::Infer(
                        pattern.location,
                ...
                let fields: Vec<_> = fields
                    ...
                    .collect();
                (
                    PatternKind::Struct(path?, fields.into_iter().collect::<Result<Vec<_>, _>>()?),
                    span,
                )
            }
            ast::ParsedPatternNodeKind::Wildcard => (PatternKind::Wildcard, span),
            ast::ParsedPatternNodeKind::Infer(name) => (
                PatternKind::Variant(
                    hir::InferOrPath::Infer(pattern.location, Some(self.intern_symbol(name))),
                    vec![],
                ),
                pattern.location,
            ),
        ...
        let kind = match expr.kind {
            ast::ExprNodeKind::Literal(literal) => hir::ExprKind::Literal(match literal {
                ...
                ast::LiteralKind::String(value) => {
                    LiteralKind::String(self.symbol_interner.intern(value))
                }
            }),
            ...
            ast::ExprNodeKind::If {
                ...
                hir::ExprKind::If(
                    ...
                    Box::new(then_branch?),
                    else_branch
                        .map(|else_branch| self.lower_expr(*else_branch).map(Box::new))
                        .map_or(Ok(None), |result| result.map(Some))?,
                )
            }
            ast::ExprNodeKind::Match { matchee, arms } => {
                ...
                hir::ExprKind::Match(
                    Box::new(matchee?),
                    arms.into_iter().collect::<Result<Vec<_>, _>>()?,
                )
            }
            ast::ExprNodeKind::Block {
                stmts,
                expr: result_expr,
            } => {
                ...
                let expr = result_expr
                    .map(|expr| self.lower_expr(*expr).map(Box::new))
                    .map_or(Ok(None), |result| result.map(Some));
                self.end_scope();
                hir::ExprKind::Block(stmts, expr?)
            }
            ast::ExprNodeKind::Function(sig, body) => hir::ExprKind::Function({
                let id = self.expect_def_id(expr.id, "Expected a function id");
                Box::new(hir::AnonFunction {
                    id,
                    function: self.lower_function(id, expr.id, sig, *body, true)?,
                })
            }),
            ast::ExprNodeKind::Print(args) => hir::ExprKind::Print(
                args.into_iter()
                    .map(|arg| self.lower_expr(arg))
                    .collect::<Vec<_>>()
                    .into_iter()
            ...
            ast::ExprNodeKind::GetPath(path) => hir::ExprKind::Path(match path.infer_path {
                ...
                InferPathKind::Path(path) => hir::PathExpr::Path(self.lower_path(&path)?),
            }),
            ...
            ast::ExprNodeKind::Property(expr, field) => {
                hir::ExprKind::Field(Box::new(self.lower_expr(*expr)?), self.intern_symbol(field))
            }
            ast::ExprNodeKind::Return(expr) => hir::ExprKind::Return(
                expr.map(|expr| self.lower_expr(*expr).map(Box::new))
                    .map_or(Ok(None), |result| result.map(Some))?,
            ),
            ast::ExprNodeKind::StructInit { path, fields } => {
                let path = match path.infer_path {
                    InferPathKind::Infer(name) => hir::InferOrPath::Infer(
                        path.location,
                        name.map(|name| self.intern_symbol(name)),
                    ),
                ...
                let fields = fields
                    ...
                    .map(|(field_name, field_expr)| {
                        ...
                        field_expr.map(|expr| hir::FieldExpr {
                            span: SourceLocation::new(
                                field_name.span.start_line,
                                expr.span.end_line,
                            ),
                            field: field_name,
                            expr,
                    ...
                    .collect::<Vec<_>>()
                    .into_iter()
                    .collect::<Result<Vec<hir::FieldExpr>, _>>()?;
            ...
            ast::ExprNodeKind::MethodCall {
                ...
                hir::ExprKind::MethodCall(
                    ...
                    hir::PathSegment {
                        res: Resolution::None,
                        ident: self.intern_symbol(method.name),
                        args: generic_args?,
                    },
                    args,
                )
    ...
    fn lower_item(
        &mut self,
        item: ast::Item,
    ) -> Result<(DefId, hir::Item, SourceLocation), LoweringErr> {
        Ok(match item {
            ast::Item::Struct(struct_def) => {
                ...
                let fields = self.lower_fields(
                    struct_def.fields,
                    self.name_info.structs[struct_id].fields.clone(),
                );
                self.end_scope();
                (
                    struct_id,
            ...
            ast::Item::Fun(function_def) => {
                ...
                let span = SourceLocation::new(
                    ...
                    self.bodies[function.body].span.end_line,
                );
                (
                    func_id,
                ...
                let methods = (|| {
                    ...
                    for mut method in impl_.methods {
                        let method = (|| {
                            ...
                            let generics = self.lower_generic_params(
                                method_id,
                                method.function.proto.generic_params.take(),
                            );
                            let function = self.lower_function(
                                method_id,
                                method.id,
                                method.function.proto.sig,
    ...
    fn lower_stmt(&mut self, stmt: ast::StmtNode) -> Result<hir::Stmt, LoweringErr> {
        let (kind, span) = match stmt {
            ...
            ast::StmtNode::Let {
                ...
                (
                    hir::StmtKind::Let(pattern?, ty.map_or(Ok(None), |ty| ty.map(Some))?, expr?),
                    span,
                )
            }
    ...
    pub fn lower(mut self, items: Vec<ast::Item>) -> Result<Hir, LoweringErr> {
        ...
        Ok(Hir {
            items: self.items,
            defs_to_items: self.def_id_map,
            bodies: self.bodies,
            body_owners: self.body_owners,
            owner_to_bodies: self.owners_to_bodies,
        })
    }
}
