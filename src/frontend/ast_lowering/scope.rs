...
impl Scope{
    ...
    pub fn bindings_iter<'a>(&'a self) ->impl Iterator<Item = (SymbolIndex,Resolution)> + use<'a>{
...
impl ScopeTree{
    ...
    pub fn get_child_scopes(&self,parent_scope:ScopeId) -> Vec<ScopeId>{
        self.all_scopes.iter().enumerate().filter_map(|(i,scope)|{
    ...
    pub fn add_scope(&mut self, scope : Scope,parent:ScopeId) -> (&mut Scope,ScopeId){
        ...
        (&mut self.all_scopes.last_mut().expect("I literally just added a scope").scope,id)
    } 
