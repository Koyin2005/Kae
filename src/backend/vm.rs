...
use super::{
    instructions::{Chunk, Constant, Instruction, Program, ProgramMetadata},
    objects::{Heap, Object},
    values::{Closure, FieldRef, Function, Record, Upvalue, Value, VariantRecord},
};
...
impl VM {
    ...
    fn load_constant(&mut self, index: usize) -> Value {
        match self.constants[index].clone() {
            ...
            Constant::NativeFunction(function) => {
                Value::NativeFunction(Object::new_native_function(&mut self.heap, function))
            }
        }
    }
    ...
    fn capture_upvalue(&mut self, location: usize) -> Object {
        for upvalue in &self.open_upvalues {
            if let Upvalue::Open {
                location: upvalue_location,
            } = upvalue.as_upvalue(&self.heap)
            {
    ...
    fn get_ref_mut(&mut self, reference: Value) -> &mut Value {
        match reference {
            Value::GlobalAddress(global) => self
                .globals
                .get_mut(&global)
                .expect("Should be a valid global"),
            Value::StackAddress(address) => self
                .stack
                .get_mut(address)
                .expect("Should be a valid stack address"),
            Value::FieldRef(field_ref) => {
                let (Value::Record(record)
                | Value::VariantRecord(VariantRecord {
                    record_data: record,
                    ..
                })) = self.get_ref_mut(field_ref.base_ref)
                else {
        ...
        match reference {
            ...
            Value::FieldRef(field_ref) => {
                ...
                | Value::VariantRecord(VariantRecord {
                    record_data: record,
                    ..
                })) = self.get_ref(&field_ref.base_ref)
                else {
                    panic!("Expect a valid record")
                };
    ...
    pub fn runtime_error(&self, message: &str) {
        ...
        for frame in self.frames.iter().rev() {
            eprintln!(
                "[line {}] in {}",
                frame.function.chunk.lines[frame.ip.min(frame.function.chunk.lines.len() - 1)],
                frame.function.name
            );
        }
    }
    pub fn run(&mut self) -> Result<(), RuntimeError> {
        let mut debug_buffer = if DEBUG_TRACE_EXEC {
            Some(String::new())
        } else {
            None
        };
        while self.current_frame().ip < self.current_chunk().code.len() {
            ...
            match self.read_instruction() {
                ...
                Instruction::LoadClosure(constant) => {
                    let Constant::Function(function) = self.constants[constant as usize].clone()
                    else {
                        ...
                        .map(|(index, is_local)| {
                            if is_local {
                                ...
                                let closure = self
                                    .current_frame()
                                    .closure
                                    .expect("Can only capture a closure.");
                                closure.as_closure(&self.heap).upvalues[index]
                            }
                        })
                        .collect();
                    let closure = Value::Closure(Object::new_closure(
                        &mut self.heap,
                ...
                Instruction::LoadField(field) => match self.pop() {
                    Value::Record(record)
                    | Value::VariantRecord(VariantRecord {
                        record_data: record,
                        ..
                ...
                Instruction::GetArrayLength => {
                    let Value::Array(array) = self.peek_ref(0) else {
                        unreachable!(
                            ...
                            self.peek_ref(0).format(&self.heap)
                        )
                    };
                ...
                Instruction::LoadLocalRef(local) => {
                    self.push(Value::StackAddress(
                        self.current_frame().bp + local as usize,
                    ))?;
                }
                ...
                Instruction::LoadUpvalue(upvalue) => {
                    let upvalue = self
                        .current_frame()
                        .closure
                        .expect("Can only use upvalues with closure")
                        .as_closure(&self.heap)
                        .upvalues[upvalue as usize]
                        .as_upvalue(&self.heap);
                ...
                Instruction::LoadIndexRef => {
                    ...
                    if index >= 0 && (index as usize) < len {
                        self.push(Value::IndexRef(Box::new(FieldRef {
                            base_ref: Value::Array(array),
                ...
                Instruction::Call(args) => {
                    ...
                    match function {
                        ...
                        Value::Closure(closure) => {
                            self.push_frame(
                                closure.as_closure(&self.heap).function.clone(),
                                arg_count,
                                Some(closure),
                            )?;
                        }
                ...
                Instruction::BuildRecord(struct_metadata) => {
                    let ProgramMetadata::Struct(struct_info) = &self.metadata[struct_metadata]
                    else {
                ...
                Instruction::BuildVariantRecord(variant_metadata) => {
                    let ProgramMetadata::Variant(variant_info) = &self.metadata[variant_metadata]
                    else {
